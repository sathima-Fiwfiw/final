...
public class itselfgame<JumpThread> extends JFrame {
    String characterID;
    String nameUser;
    String IP;
    PanelGame pInGame;
    ...
    itselfgame(String characterID, String nameUser, String IP) {
        ...
        showReadyThread = new showReady();
        pInGame = new PanelGame(this, timecount, candyFallThread, handgrost, showReadyThread);
...
class PanelGame extends JPanel implements MouseMotionListener, MouseListener {
    ...
    PanelGame(itselfgame ingame, tradetime timecount, ThreadRain candyFallThread, ThreadHand handgrost,
        ...
        for (int i = 0; i < CandyRain.length; i++) {
            CandyRain[i] = new ImageIcon("C:/oopProjactGram/candy/" + ((i % 10) + 1) + ".png").getImage();
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (showReadyThread.showReadyImage) {
            g.drawImage(ready, 250, 200, this);
        } else {
            actionTimer.start();
            for (int i = 0; i < Candy; i++) {
                if (candyFallThread.iscandy[i]) {
                    g.drawImage(CandyRain[i], candyFallThread.Ranx[i], candyFallThread.Rany[i], candyWidth, candyHeight,
                            this);
                }
            }
            ...
            if (candyFallThread.ispumpkin) {
                g.drawImage(pumpkin, candyFallThread.pumpkinX, candyFallThread.pumpkinY, pumpkinWidth, pumpkinHeight,
                        this);
            }
            g.drawImage(currentCharacter, movecharacter - getCharacterOffset(), characterY, getCharacterWidth(),
                    characterHeight, this);
    ...
    void checkCollectCandy(int index) {
        ...
        if (candyFallThread.iscandy[index] && candyBottom >= characterTop && candyRight >= characterLeft - 75
    ...
    void checkCollectDonut() {
        ...
        int characterRight = movecharacter + getCharacterWidth();
        int characterTop = characterY;
        if (candyFallThread.isdonut && DonutBottom >= characterTop && DonutRight >= characterLeft - 75
    ...
    void checkCollectPumpkin() {
        ...
        if (candyFallThread.ispumpkin && pumpkinBottom >= characterTop && pumpkinRight >= characterLeft - 75
