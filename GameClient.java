...
public class GameClient extends JFrame {
    private static final int SERVER_PORT = 32499; // พอร์ตที่ใช้ในการเชื่อมต่อกับเซิร์ฟเวอร์
    ...
    void loadImages() {
        ...
        for (int i = 0; i < Candys; i++) {
            Candy[i] = new ImageIcon("C:/oopProjactGram/candy/" + ((i % 10) + 1) + ".png").getImage();
        }
    ...
    private void createAndShowGUI() {
        ...
        panel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                if (isReady) {
                    ...
                    g.drawImage(character[getCharacterIndex(characterCode)],
                            movecharacter - getCharacterOffset(characterCode), characterY,
                            getCharacterWidth(characterCode), 250, this); // วาดตัวผู้เล่น
                    ...
                    for (Map.Entry<String, PlayerPoint> entry : otherPlayers.entrySet()) {
                        ...
                        Image otherCharacterImage = getCharacterImageBasedOnCode(p.characterCode); // ใช้ characterCode
                                                                                                   // ของผู้เล่นอื่น
                        g.drawImage(otherCharacterImage, p.x - getCharacterOffset(p.characterCode), p.y,
                                getCharacterWidth(p.characterCode), 250, this); // วาดตัวผู้เล่น
                        g.setFont(font);
                ...
                if (!remainEnd) {
                    if (showTimerUPimage) {
                        ...
                        if ("c01".equals(winnerCharacterCode)) {
                            g.drawImage(character[getCharacterIndex(winnerCharacterCode)], 500, 250,
                                    getCharacterWidth(winnerCharacterCode) - 70, 250 - 70, this);
                        } else if ("c05".equals(winnerCharacterCode)) {
                            g.drawImage(character[getCharacterIndex(winnerCharacterCode)], 500, 250,
                                    getCharacterWidth(winnerCharacterCode) - 40, 250 - 70, this);
                        } else {
                            g.drawImage(character[getCharacterIndex(winnerCharacterCode)], 520, 250,
                                    getCharacterWidth(winnerCharacterCode) - 40, 250 - 70, this);
                        }
                        xbtt = 650;
                        ybtt = 550;

                        // สร้างปุ่มใหม่และเพิ่มลงใน panel
                        MenuButton.setBounds(xbtt, ybtt, 150, 50); // ตั้งตำแหน่งปุ่ม
                        panel.add(MenuButton); // เพิ่มปุ่มลงใน panel

                        // เรียกใช้ revalidate และ repaint เพื่ออัปเดตหน้าจอ
                        panel.revalidate();
                        panel.repaint();
                    }
        ...
        ImageIcon Menu = new ImageIcon(System.getProperty("user.dir") +
                File.separator + "ScoreImg" + File.separator + "buttonScore.png");
        Image MenuButtonImage = Menu.getImage().getScaledInstance(150, 50, Image.SCALE_SMOOTH);

        MenuButton = new JButton(new ImageIcon(MenuButtonImage));
        MenuButton.setBounds(xbtt, ybtt, 150, 50); // ตั้งตำแหน่งปุ่ม
    ...
    private void connectToServer() {
        try {
            ...
            out = new PrintWriter(socket.getOutputStream(), true); // สร้าง PrintWriter
                                                                   // เพื่อส่งข้อมูลออกไปยังเซิร์ฟเวอร์
    ...
    private void sendPosition() {
        out.println("player," + playerName + "," + movecharacter + "," + characterY + "," + characterCode); // ส่งชื่อผู้เล่น
                                                                                                            // ตำแหน่ง
                                                                                                            // x, y
                                                                                                            // และโค้ดตัวละคร
    }
    ...
    void checkCollectCandy(int index) {
        ...
        if (iscandy[index] && candyBottom >= characterTop && candyRight >= characterLeft - 75
                && candyLeft <= characterRight - 125) {
            // ลูกอมชนกับตัวละคร
            iscandy[index] = false;
            Score += 50;
    ...
    void checkCollectDonut() {
        ...
        int characterRight = movecharacter + getCharacterWidth(characterCode);
        int characterTop = characterY;
        if (isdonut && DonutBottom >= characterTop && DonutRight >= characterLeft - 75
                && DonutLeft <= characterRight - 125) {
    ...
    void checkCollectPumpkin() {
        ...
        if (ispumpkin && pumpkinBottom >= characterTop && pumpkinRight >= characterLeft - 75
                && pumpkintLeft <= characterRight - 125) {
