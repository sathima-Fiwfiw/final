...
public class GameServer {
    private static final int SERVER_PORT = 32499;
    ...
    // เมธอดในการเพิ่มผู้เล่นใหม่
    public void addNewPlayer(String playerName, String characterID, PrintWriter newPlayerOutput) {
        // ตรวจสอบว่าชื่อผู้เล่นมีอยู่แล้วหรือไม่
        if (playerNames.contains(playerName)) {
            newPlayerOutput.println("Username already taken. Please choose another name."); // แจ้งผู้เล่น
            return; // ออกจากฟังก์ชัน
        }

        // สร้าง Player ใหม่และเพิ่มลงในลิสต์
        Player newPlayer = new Player(playerName, characterID, newPlayerOutput);
        players.add(newPlayer);
        playerNames.add(playerName); // เพิ่มชื่อผู้เล่นใหม่ลงในลิสต์

        // ส่งข้อมูลผู้เล่นทั้งหมดไปยังผู้เล่นใหม่
        for (Player player : players) {
            String message = "waitplay," + player.getName() + "," + player.getCharacterID();
            newPlayerOutput.println(message); // ส่งข้อมูลให้ผู้เล่นใหม่
        }

        // ส่งข้อมูลผู้เล่นใหม่ไปยังผู้เล่นที่มีอยู่
        for (Player player : players) {
            if (!player.getOutput().equals(newPlayerOutput)) { // ไม่ส่งกลับไปยังผู้เล่นใหม่
                String message = "waitplay," + playerName + "," + characterID;
                player.getOutput().println(message); // ส่งข้อมูลให้ผู้เล่นที่มีอยู่
            }
        }
    }
    ...
    private boolean isBroadcastingReady = false;

    private void startBroadcastingReady() {
        if (isBroadcastingReady)
            return; // ป้องกันการเริ่มหลายครั้ง

        isBroadcastingReady = true;
        ready = new showReady();
        ...
        new Thread(() -> {
            while (timecount.isend) {
                try {
                    ...
                    // รอ 500ms เพื่อลดการใช้ทรัพยากร
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
    ...
    private class ClientHandler extends Thread {
        ...
        public void run() {
            try {
                ...
                characterCodewait = in.readLine();
                int characterIndex = getCharacterIndex(characterCodewait); // ใช้เมธอดใหม่ในการแปลงรหัส
                ...
                // sendPlayerList(); // ส่งข้อมูลผู้เล่นคนอื่นให้ผู้เล่นใหม่

                // เพิ่มผู้เล่นใหม่
                addNewPlayer(playerName, characterCodewait, out);

                // แจ้งให้ผู้เล่นคนอื่นรู้ว่าเข้ามาแล้ว
                // broadcast("waitplay,"+playerName + "," + characterCodewait);
        ...
        private void sendPlayerList() {
            for (Player player : players) {
                out.println("waitplay," + player.getName() + "," + player.getCharacterID());
            }
        }
    ...
    public class Player {
        ...
        public Player(String name, String characterID, PrintWriter output) {
            this.name = name;
